  ...
  return (
    <div className="flex flex-col items-center gap-5 sm:gap-12 p-3 sm:p-12 border border-[#2b2b2b] rounded-3xl">
      <div className="relative w-full">
        <img className="w-full rounded-[20px]" src={project.image} alt="" />
      ...
      <div className="flex flex-row items-center justify-between w-full">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden text-lg text-gray-600 sm:block">
            Show<span> {isShow ? 'Less' : 'More'}</span>
          </div>
          <div
            className="p-[1px] rounded-full bg-gradient-to-t from-transparent to-[#29334a] cursor-pointer"
            onClick={showContent}
          >
        ...
        <>
          <div className="flex flex-col items-start w-full gap-3 md:items-center md:flex-row md:flex-wrap">
            {project.tags.map((tagObj, index) => (
              <div
                key={index}
                className="flex items-center gap-2 rounded-[100px] border border-[#2b2b2b] py-4 px-[18px]"
              >
                {tagObj.icon}
                <div className="text-sm sm:text-lg text-[#A6A6A6]">
                  {tagObj.tag}
                </div>
              </div>
            ))}
          ...
          {/* Project Description */}
          <div className="flex flex-col items-center w-full gap-12 rounded-3xl">
            ...
            <div className="flex flex-col items-center w-full gap-7">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-5 sm:gap-12 px-5 sm:px-12 py-3 sm:py-8  rounded-xl w-full">
                ...
                <div className="flex flex-wrap items-center gap-3 sm:gap-12">
                  {project.stacks.map((stack, index) => (
                    ...
                    >
                      <div
                        id={`tool-tip-${index}`}
                        className="flex items-center justify-center p-1 sm:p-3 rounded-full bg-gradient-to-t to-[#141a29] from-transparent"
                      >
                        {stack}
                      </div>
                      <Tooltip anchorSelect={`#tool-tip-${index}`}>
                        hello world
                      </Tooltip>
                    </div>
                  ))}
              ...
              {/* <div className="w-full flex flex-col p-5 sm:p-12 gap-3 sm:gap-7 border border-[#2b2b2b] rounded-xl">
                ...
                <div className="flex flex-wrap lg:justify-between gap-1 sm:gap-0 md:gap-5 border-0 sm:border border-[#2b2b2b] rounded-xl sm:py-7 sm:px-12">
                  {project.methods.map((method, index) => (
                    <div
                      key={index}
                      className="py-2 sm:py-4 px-2 sm:px-8 border border-[#2b2b2b] rounded-3xl text-white text-xs sm:text-base"
                    >
                      {method}
                    </div>
                  ))}
